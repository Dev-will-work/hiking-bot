...
Но точно то, что воды теперь всем хватит в походе, и вещей вы один как 10 человек привезёте,
 ну и заодно людей поближе к месту похода довезёте 🧳🌊""",
           'food': """Опытный повар вроде 👨‍🍳
Это понятно, что в походе пасту Болоньезе никто не даст, максимум томатную пасту сами туда возьмёте..
...
*Через несколько часов, когда все ушли за дровами и брёвнами*
...
Но не грусти, мы подготовили для тебя короткий смешной тест о том, кто ты обычно в походе 🤠
...
def continuation_handler(update: Update, _: CallbackContext):
    ...
    if result == "question":
        keyboard = [
            [
                InlineKeyboardButton("Не, не хочу!", callback_data="answer random"),
                InlineKeyboardButton("Да, давай 🙂", callback_data="answer test"),
            ]
        ]
        message_content = "Начать тест?"
    elif result == "exact":
        keyboard = [
            [
            ...
            [
                ...
                InlineKeyboardButton("Я за брёвнами", callback_data="answer axe"),
            ]
        ]
